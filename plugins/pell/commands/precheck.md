...
- **Ticket key** — first match of `\b[A-Z][A-Z0-9]+-\d+\b`. If present, capture as `self_key`: it seeds the query text (Step 4), is excluded from its own match set, and is the write target for Step 7.
- **Free text** — if no key is found, the entire argument string is `query_text` (a ticket the user is about to file). There is no write target, so Step 7 is skipped.
- **Scope modifiers** (case-insensitive; strip from the free text before it becomes `query_text`):
  ...
  - `skip repo` / `skip bitbucket` / `skip git` → suppress that signal in Step 5.
...
## Step 4 — Resolve query text and target project
...
## Step 7 — Side effects (existing-key path only)

Offer this **only** when `self_key` is present and there is at least one `likely-dupe` Jira match. If `self_key` is absent (free-text path), end after Step 6 — there is nothing to write to.
...
- Resolve the duplicate link type's exact name (commonly `Duplicate` — outward `duplicates`, inward `is duplicated by`). The tool differs by connection shape — name both, by role, and use whichever the session exposes: `listJiraIssueLinkTypes` on `plugin:atlassian:atlassian`, which is not a primary tool there — reach it through `discover`, then run `executeRead({name: "listJiraIssueLinkTypes", cloudId, inputs: {}})`; `getIssueLinkTypes` on the classic connection, called directly with `cloudId`.
- Create the link. The tool and its link-type parameter name both differ by connection shape: `createJiraIssueLink` on `plugin:atlassian:atlassian`, reached through `discover` and run with `executeWrite({name: "createJiraIssueLink", cloudId, inputs: {linkType: <that name>, inwardIssue: <self_key>, outwardIssue: <match_key>}})`; `createIssueLink` on the classic connection, called directly with `cloudId`, `type: <that name>`, `inwardIssue: <self_key>`, `outwardIssue: <match_key>`. On both execute calls `cloudId` is top-level, never inside `inputs`. The stored link reads "`<self_key>` duplicates `<match_key>`": both tools read a link from `inwardIssue` to `outwardIssue` using the type's outward wording, so the *acting* issue is the `inwardIssue` (their own example: `Blocks` with `inwardIssue: B`, `outwardIssue: A` means B blocks A). If this site's link-type descriptions imply the opposite mapping, follow those so the human-readable direction is preserved.
- Print `Linked.`.

**Comment — once, after the link prompts.** If there was at least one `likely-dupe` match, prompt `Comment on <self_key> noting the suspected duplicate(s) / existing implementation? (y/n)`. On `y`: post it. The tool differs by connection shape — name both, by role, and use whichever the session exposes: `mcp__plugin_atlassian_atlassian__addOrEditJiraIssueComment` on `plugin:atlassian:atlassian` (omit `commentId` — with it the tool edits an existing comment instead of adding one); `addCommentToJiraIssue` on the classic connection. Both take `cloudId`, `issueIdOrKey: self_key`, `contentFormat: "markdown"`, and a `commentBody` listing the matched tickets / PRs / implementation with links. Print `Commented.`.

Each write is independent and individually gated. There is no pre-authorization shortcut in v1.

## Step 8 — Operator notes

- **Read-only by default.** The only writes are the Step 7 link/comment, each `(y/n)`-gated. **Never** resolve, close, transition, or edit ticket fields — duplicate handling is a human decision; `precheck` only flags and, on consent, links/comments.
