...
**Draft comment** per ticket, built from this ticket's own `new` findings plus this ticket's `new`-status collision items from `collision_pairs` (never `asked` or `answered` — those already had their turn, whether the status came from this ticket's own auditor or defaulted to `new` because only the other side's auditor flagged it):
...
Section contents. For every category except `collision`, "new" means this ticket's own finding's `thread_status` is `new`. For `collision`, treat each of this ticket's `collision_pairs` entries as one synthetic item at the pair's reference `severity`/`code_note`/`question`; "new" means this ticket's own collision status for that pair is `new`; `<KEY>` in its text is always the *other* ticket in the pair — never this ticket's own key, which fixes the earlier bug where the merge could leave a ticket's key naming itself.

- **Questions** — numbered: the `question` from every new `blocker` (finding or collision item, any category), then every new `major` (finding or collision item, any category), then the `question` from every new `minor` `business-gap` finding.
- **Suggested approach** — the `suggestion` from every new `approach` finding, one bullet each.
- **Code notes** — one bullet per non-empty `code_note`: a plain bullet for every new `blocker`, `major`, or `minor` finding of any category except `collision`; then `Overlaps with <KEY>: <code_note>` for every new `blocker`, `major`, or `minor` collision item.
