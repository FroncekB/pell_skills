...
The `Epic:` line has four alternative forms when there is no home epic: `Unparented — suggested epic: <KEY> — <summary>`, `Unparented — fits no scope statement`, `This ticket is an epic`, and `<placement_note>` when placement is unassessed (Section 7 outcomes 3 and 4). `Scope:` is omitted when placement is unassessed or the ticket is an epic. `Siblings:` is omitted when unparented, unassessed, or the ticket is an epic. `Ticket links:` renders the remote links captured in Section 6 and is omitted when there are none.
...
- **Cache commit gate.** `/pell:scope` may have just written `docs/pell/sow-<PROJECT>.md`, and `/pell:start-work`'s pre-flight refuses a dirty tree. Unless Step 4 is already being skipped, Step 2.5 ends by running `git status --porcelain`, unscoped — `/pell:start-work`'s own pre-flight (Section 3 of that command) is unscoped, so a path-scoped check here would miss other dirty files and let `start-work` refuse anyway. Three cases: if the output is exactly one line naming `docs/pell/sow-<PROJECT>.md`, prompt to commit exactly that file on the current branch `(y/n)` — on `y`, `git add` the one path and commit it as `docs(pell): add synthesized SOW for <PROJECT>`; on `n`, treat `skip start-work` as set and tell the user to commit or stash it themselves. If the output names that file and other files too, commit nothing, print that the tree has other uncommitted changes, and treat `skip start-work` as set. If the output is empty or does not mention that file, the gate does not apply and Step 2.5 continues. This is the only commit `from-ticket` ever makes.
